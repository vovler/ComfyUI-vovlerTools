...
class SDXLDirectoryToOnnx:
    ...
    separate, optimized ONNX files. This version uses the correct, consistent
    precision and GPU optimization settings.
    """
    OUTPUT_NODE = True
    CATEGORY = "Export"
    ...
    def optimize_model(self, model_path: Path, component_name: str, optimization_level: int, use_fp16: bool):
        """Helper to optimize a saved ONNX model with correct precision and hardware settings."""
        try:
            print(f"[INFO] ONNX Exporter: Optimizing {component_name} for GPU: {self.gpu_available}...")
            optimizer = ORTOptimizer.from_pretrained(model_path.parent)
            
            # THE KEY FIX: Create an OptimizationConfig that respects the user's choices.
            optimization_config = OptimizationConfig(
                optimization_level=optimization_level,
                fp16=use_fp16,
                # Enable hardware-specific optimizations for the available device.
                optimize_for_gpu=self.gpu_available 
            )
    ...
    def export_from_directory(self, model_directory, output_subfolder_name, optimization_level, use_fp16,
        ...
        try:
            ...
            if export_unet:
                ...
                self.optimize_model(unet_path / "model.onnx", "UNet", optimization_level, use_fp16)
            ...
            if export_clip:
                ...
                self.optimize_model(text_encoder_path / "model.onnx", "Text Encoder 1", optimization_level, use_fp16)
                ...
                self.optimize_model(text_encoder_2_path / "model.onnx", "Text Encoder 2", optimization_level, use_fp16)
            ...
            if export_vae:
                ...
                self.optimize_model(vae_encoder_path / "model.onnx", "VAE Encoder", optimization_level, use_fp16)
                ...
                self.optimize_model(vae_decoder_path / "model.onnx", "VAE Decoder", optimization_level, use_fp16)
